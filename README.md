...
**Версия 3.4** — API только с флагом `--api`; исправлен выбор длины маршрута в неоднозначных пакетах (в т.ч. 2B/3B), улучшен debug-лог (`RAW: <hex>` + корректная привязка к hash).
