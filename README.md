...
- **Таблица входящих соседей** — кто доставляет пакеты ретранслятору (->RPT) и наблюдателю (->OBS)
- **Таблица исходящих соседей** — через кого уходят пакеты от ретранслятора (API или ответы ботов)
- **Отладка ->OBS** — подсветка и логирование пакетов, принятых observer напрямую (`-d`)
...
| `-d`, `--debug` | Логировать пакеты ->OBS в файл `meshcore-debug.log` |
