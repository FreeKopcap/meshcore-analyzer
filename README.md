...
**Версия 3.1** — независимый от USB поллер API MeshCoreTel (`--api`), поддержка маршрутов с 2 байтами на хоп (прошивки Meshcore 1.14+).

## Возможности

-- **Исходящие соседи через API** — получение данных из [MeshCoreTel](https://meshcoretel.ru) без отправки сообщений в каналы (`--api`). При потере USB-порта API-прослушка продолжает работать и обновлять исходящих соседей.
...
# Все отчёты + API + отладка ->OBS
uv run meshcore-analyzer.py -vnd --hops --api
...
| `-d`, `--debug` | Логировать все пакеты в файл `meshcore-debug.log` |
